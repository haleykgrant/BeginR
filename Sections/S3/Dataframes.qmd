...
In this chapter, we're going to go over manipulating data frames in R, including:
...
We're going to use the `cdc` data from the previous chapter as an example. Here's a reminder of how we read in that dataset (this code will automatically run when the webpage is ready):
...
## Making changes to a dataframe {#sec-data_manipulation}
