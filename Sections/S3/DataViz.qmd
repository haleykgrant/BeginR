...
In this chapter, we will cover data visualization in R using the `ggplot2` package (one of the packages included in the `tidyverse`). Specifically, we will learn about
...
Let's see some examples! We will use the `census.rda` data from [Chapter 7](DataImport.qmd). I have already loaded the data for you. Reminder: the data frame is saved under the name `census`. Here's a reminder of what the data look like:
